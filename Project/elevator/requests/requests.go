...
func RequestsShouldClearImmediately(e elev.Elevator, btn_floor int, btn_type elevio.Button) bool {
	...
	case elev.CRVAll:
		logrus.Warn("CRVAll, RequestsShouldClearImmediately")
		return e.CurrentFloor == btn_floor
	case elev.CRVInDirn:
		logrus.Warn("CRVInDirn, RequestsShouldClearImmediately")
		return e.CurrentFloor == btn_floor &&
...
func RequestsClearAtCurrentFloor(e elev.Elevator) elev.Elevator {
	switch e.Config.ClearRequestVariant {
	case elev.CRVAll:
		logrus.Warn("CRVAll, RequestsClearAtCurrentFloor")
		for btn := 0; btn < elevio.NButtons; btn++ {
			e.Requests[e.CurrentFloor][btn] = false
		}

	case elev.CRVInDirn:
		logrus.Warn("CRVInDirn, RequestsClearAtCurrentFloor")
